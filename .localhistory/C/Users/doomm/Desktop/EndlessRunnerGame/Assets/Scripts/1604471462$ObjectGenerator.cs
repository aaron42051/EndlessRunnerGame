...
public class ObjectGenerator : MonoBehaviour {
    ...
    {
        if (Random.Range(0f, 100f) < randomPincerThreshold)
        {
            GameObject newPincer = pincerPool.ActivatePoolObject();

            float pincerXPosition = Random.Range(-platformWidths[platformSelector] / 2 + 1f, platformWidths[platformSelector] / 2 - 1f);

            Vector3 pincerPosition = new Vector3(pincerXPosition, 0.5f, 0f);
